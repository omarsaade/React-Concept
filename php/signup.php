...
if (!empty($fname) && !empty($lname) && !empty($email) && !empty($password)) {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) { //if email is valid,
        ...
        // echo "Thr email is inserted correctly";
        ...
        if (mysqli_num_rows($sql) > 0) { //if email already exist
            ...
            if (isset($_FILES['image'])) { //if file is uploaded
                ...
                if (in_array($img_ext, $extensions) === true) { // if user uploaded img ext is matched with any array extensions
                    ...
                    if (move_uploaded_file($tmp_name, "images/" . $new_img_name)) { //if user uoload img move to our folder successfully
                        $status = "Active now"; //once user signed up then his status will be acive now
